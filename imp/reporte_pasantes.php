...
$query = "SELECT nombre, apellido, cedula, aprobada, m01_registrada ::timestamp::date, m02_aceptada ::timestamp::date, m03_numero_asignado ::timestamp::date, m04_sellada::timestamp::date, m05_entrego_copia ::timestamp::date, m06_entrego_borrador ::timestamp::date, m07_retiro_borrador ::timestamp::date,	m08_entrega_final ::timestamp::date, m09_carga_nota ::timestamp::date, aprobada FROM usuario INNER JOIN pasantia ON usuario.id = pasantia.usuario_id AND usuario.tipo = 'estudiante' ORDER BY cedula";
